...
## Generating LLM quizzes

Mechanical masking can't make conceptual questions or summaries. Those are
generated by an LLM and the validated output is cached in
[`quizgen/cache/llm/`](quizgen/cache/llm/) and committed, so `generate` can merge
it without ever calling a model. Pick a provider with `--provider`.

### Provider: local ollama (`--provider ollama`, default)

Runs on your machine with [ollama](https://ollama.com) — no API key, no cost.

```sh
ollama pull qwen2.5-coder:7b           # any model you have
export GITHUB_TOKEN=$(gh auth token)   # issue fetch needs a token
cd quizgen
go run ./cmd/quizgen llm-generate --model qwen2.5-coder:7b --max-proposals 50
```

### Provider: Cloudflare Workers AI (`--provider workers-ai`)

Runs the model in the cloud (pay-per-use; the daily free tier covers small
batches). Set the Cloudflare account ID and an API token via env vars:

```sh
export GITHUB_TOKEN=$(gh auth token)
export CLOUDFLARE_ACCOUNT_ID=...          # or pass --cf-account-id
export CLOUDFLARE_API_TOKEN=...           # token with Workers AI access
cd quizgen
go run ./cmd/quizgen llm-generate \
  --provider        workers-ai \
  --model           @cf/meta/llama-3.3-70b-instruct-fp8-fast \
  --max-generations 30          # stop after 30 new generations (stay in the free tier)
```

`--max-generations` caps how many *new* generations one run performs — run it
day by day to backfill entirely within the free allocation. Then commit:
...
regenerate). Every generated quiz is validated — each answer must appear verbatim
